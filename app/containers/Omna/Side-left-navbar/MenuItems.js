...
const DEFAULT_MENU_ITEMS = [
    {
        ...
        subItems:
            [{ icon: <VerticalSplitIcon />, text: 'Orders', link: '/orders' }]
    },
    {
        text: "Settings", icon: <SettingsIcon />, link: "", expandableItem: true,
        subItems: [
...
const Item = ({ icon, text, link, classes, open, expandableItem, subItems = [], onClick }) => (
    <Fragment>
        {expandableItem ? <ListItem button onClick={onClick}>
            <ListItemIcon>
                {icon}
            </ListItemIcon>
        ...
        <Collapse in={open} timeout="auto" unmountOnExit>
            {subItems && subItems.map(({ icon, text, link }, key) => (
                <List key={key} component="div" disablePadding>
                    <ListItem button className={classes.nested} component={Link} to={link}>
                        <ListItemIcon>
                            {icon}
                        </ListItemIcon>
                        <ListItemText inset primary={text} />
...
class MenuItems extends Component {
    ...
    render() {
        ...
        return (
            <div>
                <List>
                    {DEFAULT_MENU_ITEMS && DEFAULT_MENU_ITEMS.map(({ text, icon, link, expandableItem, subItems }, index) => (
                        <Item key={index}
                            icon={icon}
                            text={text}
                            link={link}
